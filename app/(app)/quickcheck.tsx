...
import { getScannerOptions, runExactSystemQuickCheck, runQuickCheck, searchComponents, searchDevices, searchScanners, type QuickCheckItem, type QuickCheckResult } from '@/lib/quickcheck';
...
export default function QuickCheck() {
  ...
  const [selectedComponents, setSelectedComponents] = useState<QuickCheckItem[]>([]);
  ...
  function toggleComponent(item: QuickCheckItem) {
    setSelectedComponents((current) => current.some((x) => x.id === item.id) ? current.filter((x) => x.id !== item.id) : [...current, item]);
  }

  async function check() {
    if (!device || !scanner || !strength) return;
    setLoading(true); setError('');
    try {
      const response = selectedComponents.length
        ? await runExactSystemQuickCheck({ device_id: device.id, component_ids: selectedComponents.map((x) => x.id), scanner_model_id: scanner.id, scanner_strength_t: strength, scan_region: region })
        : await runQuickCheck({ device_id: device.id, scanner_model_id: scanner.id, scanner_strength_t: strength, scan_region: region });
      setResult(response); setStep(5);
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to complete the safety check.'); }
    finally { setLoading(false); }
  }

  function reset() { setStep(1); setDevice(null); setSelectedComponents([]); setScanner(null); setStrength(null); setResult(null); setError(''); setDeviceSearch(''); setComponentSearch(''); setScanners([]); }
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.content}>
    ...
    {step === 1 && <Card><SectionTitle title="Identify the implant" subtitle="Search by manufacturer, model, or catalog number. Select the exact device."/><SearchInput value={deviceSearch} onChangeText={setDeviceSearch} placeholder="Search implant or model…" autoFocus/>{devices.slice(0, 8).map((item) => <Choice key={item.id} label={label(item)} selected={device?.id === item.id} onPress={() => { setDevice(item); setSelectedComponents([]); setStep(2); }}/>)}</Card>}
    {step === 2 && <Card><SectionTitle title="Confirm exact components" subtitle="Select every implanted lead/component shown in the patient's documentation. Compatibility is never inferred from separate models."/><Text style={styles.selectedLabel}>{label(device!)}</Text><SearchInput value={componentSearch} onChangeText={setComponentSearch} placeholder="Search lead or component…"/>{components.slice(0, 12).map((item) => <Choice key={item.id} label={label(item)} selected={selectedComponents.some((x) => x.id === item.id)} onPress={() => toggleComponent(item)}/>)}<View style={styles.selectionSummary}><Text style={styles.selectionSummaryTitle}>{selectedComponents.length ? `${selectedComponents.length} component${selectedComponents.length === 1 ? '' : 's'} selected` : 'No component selected'}</Text><Text style={styles.selectionSummaryText}>For a complete system, select all applicable implanted components. If the implant truly has no separate component, continue with none.</Text></View><Choice label="No separate component / not applicable" selected={selectedComponents.length === 0} onPress={() => setSelectedComponents([])}/><PrimaryButton label="CONTINUE" disabled={components.length > 0 && !selectedComponents.length} onPress={() => setStep(3)}/></Card>}
...
function ResultScreen({ result, onAgain }: { result: QuickCheckResult; onAgain: () => void }) {
  ...
  const checkedComponents = Array.isArray(result.components) ? result.components : [];
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.content}>
    ...
    <View style={[styles.resultHero, { borderColor: meta.border }]}><View style={[styles.statusPill, { backgroundColor: meta.background }]}><Text style={[styles.statusPillText, { color: meta.text }]}>{meta.label}</Text></View><Text style={styles.resultHeadline}>{meta.headline}</Text><Text style={styles.resultDecision}>{result.decision || meta.defaultDecision}</Text>{strengthMismatch ? <View style={styles.mismatch}><Text style={styles.mismatchTitle}>SCANNER FIELD-STRENGTH MISMATCH</Text><Text style={styles.mismatchText}>The selected {scannerStrength}T scanner is outside the labeled field strength range: {allowed.join('T, ')}T.</Text></View> : null}</View>
    <Card><SectionTitle title="Checked configuration"/><Info label="Implant" value={displayObject(result.device)}/>{checkedComponents.length ? <View style={styles.componentList}><Text style={styles.infoLabel}>LEAD / COMPONENTS</Text>{checkedComponents.map((item, index) => <Text key={String(item.id ?? index)} style={styles.componentListItem}>{displayObject(item)}</Text>)}</View> : result.component ? <Info label="Lead / component" value={displayObject(result.component)}/> : null}<Info label="Scanner" value={displayObject(result.scanner)}/><Info label="Field strength" value={Number.isFinite(scannerStrength) ? `${scannerStrength}T` : 'Not verified'}/><Info label="Scan region" value={String(result.condition?.scan_region ?? 'Selected region')}/></Card>
...
function extractConditions(condition?: Record<string, unknown> | null) {
  ...
  const labels: Record<string, string> = { field_strength: 'Field strength', scan_region: 'Scan region', allowed_scan_regions: 'Allowed scan regions', magnet_type: 'Magnet type', bore_type: 'Bore type', scan_mode: 'Scan mode', coil_requirements: 'Coil requirements', sar_limit: 'SAR limit', b1_rms_limit: 'B1+rms limit', gradient_limit: 'Gradient limit', spatial_gradient_limit: 'Spatial gradient limit', other_conditions: 'Manufacturer conditions', patient_position: 'Patient position', device_position: 'Device position', system_requirements: 'System requirements' };
  const output: string[] = [];
  for (const [key, raw] of Object.entries(condition)) {
    if (ignored.has(key) || raw === null || raw === undefined || raw === '') continue;
    if (Array.isArray(raw)) { if (raw.length) output.push(`${labels[key] ?? prettify(key)}: ${raw.map(String).join(', ')}`); }
    else if (typeof raw === 'object') { const text = Object.entries(raw as Record<string, unknown>).map(([k, v]) => `${prettify(k)}: ${Array.isArray(v) ? v.join(', ') : String(v)}`).join('; '); if (text) output.push(`${labels[key] ?? prettify(key)}: ${text}`); }
    else if (typeof raw === 'boolean') output.push(`${labels[key] ?? prettify(key)}: ${raw ? 'Yes' : 'No'}`);
    else output.push(`${labels[key] ?? prettify(key)}: ${String(raw)}`);
  }
  return output;
}
